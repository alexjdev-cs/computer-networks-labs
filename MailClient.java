/****************************************************************************
* Program:
*    Lab MailClient, Computer Networks CSE354
* Author:
*    Alex Johnson
* Summary:
*    This program will implement a simple email client for sending email
*    messages using SMTP over TCP.
************************************************************************** */
...
class SMTPConnection
{
   ...
   private static final int SMTP_CLOSE = 221;
   private static final int SMTP_OK = 250;
   private static final int SMTP_PROCEED = 354;
   ...
   public SMTPConnection(Envelope envelope) throws IOException
   {
      ...
      final String msg = "\nInitiating SMTP over TCP...";
      System.out.println(msg);
      
      final String reply = fromServer.readLine();
      final int responseCode = parseReply(reply);

      if (responseCode != SMTP_READY)
      {
         throw new IOException("Error: Service not ready (code other than 220)");
      }
      ...
      final String cmd = "HELO " + localhost;
      
      sendCommand(cmd, SMTP_OK);
   ...
   public void send(Envelope envelope) throws IOException
   {
      ...
      final String cmdMailFrom = "MAIL FROM: " + "<" + envelope.Sender + ">";
      final String cmdRcptTo = "RCPT TO: " + envelope.Recipient;
      final String cmdData = "DATA";
      final String message = envelope.Message.toString();
      
      
      sendCommand(cmdMailFrom, SMTP_OK);
      sendCommand(cmdRcptTo, SMTP_OK);
      sendCommand(cmdData, SMTP_PROCEED);

      sendCommand(message + CRLF + ".", SMTP_OK);
   }
   ...
   public void close()
   {
      ...
      try
      {
         sendCommand("QUIT", SMTP_CLOSE); //Closes the SMTP connection
         connection.close(); //Closes the TCP socket
      }
      catch (IOException e)
      {
   ...
   private int parseReply(String reply)
   {
      ...
      code = Integer.parseInt(stringTokenizer.nextToken());
